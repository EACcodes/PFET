...
def etot(Vemb_real):
    ...
    os.system("../../../bin/molpro sys1-11.com >sys1-11")
    os.system("../../../bin/molpro sys2-22.com >sys2-22")
    ...
    for i in range(0,len(myden1)):
        mygrad[i]=5*(myden1_new[i]-myden1[i])+5*(myden2_new[i]-myden2[i])
    ...
    e_int=j_int+xc_int+ek_int-e_ion1-e_ion2+e_ion-e_emb 
...
def setup():
   newfile=file('vemb_real.txt','r')
   Vemb_real = np.loadtxt(newfile)
   a = 115*115
   b = 27115
#       a=a+1
...
#   fmin(etot,Vemb_real,maxiter=2,disp=1)
   result = minimize(etot,Vemb_real,method='BFGS', jac=True,tol=1e-5,options={'disp': True})
#   etot(Vemb_real)
